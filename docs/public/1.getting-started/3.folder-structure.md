...
- **`values-<name>.yaml`** — per-deploy overrides that enable specific tools (named after the `--name` layer identifier, e.g. `values-webshop.yaml`)
...
- **`kr`** — standalone CLI tool for bootstrapping (`kr init`) and deploying (`kr deploy`) the platform
