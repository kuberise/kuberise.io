...
- **`values-base.yaml`** — definitions of all ArgoCD Applications (disabled by default), including external chart references (`chart`, `repoURL`, `targetRevision`)
- **`values-{clusterName}-{layerName}.yaml`** — enabler files that enable specific tools per cluster and layer (e.g. `values-dev-platform.yaml`). These live in the client repo.
...
- **`kr`** — standalone CLI tool for the full platform lifecycle (`kr up`, `kr init`, `kr deploy`, `kr uninstall`)
- **`install-kr.sh`** — curl-based installer for downloading `kr` from GitHub releases
