...
| `--name` | App-of-apps name, used to deploy multiple layers (default: `app-of-apps`) |
...
kr deploy --context k3d-dev --cluster dev-app-onprem-one \
  ...
  --name app-of-apps --revision v0.3.0 --domain dev.kuberise.dev

# Pro layer
kr deploy --context k3d-dev --cluster dev-app-onprem-one \
  --repo https://github.com/kuberise/kuberise-pro.git \
  --name app-of-apps-pro --revision v0.3.0 --domain dev.kuberise.dev \
  --token $TOKEN

# Client layer
kr deploy --context k3d-dev --cluster dev-app-onprem-one \
  --repo https://github.com/org/client.git \
  --name app-of-apps-client --revision main --domain dev.kuberise.dev \
  --token $TOKEN
```
